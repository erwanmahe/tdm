...
(function(){
  ...
  backdrop.innerHTML = '<div class="lb-container">\n  <img class="lb-img" alt="" />\n  <button class="lb-close" aria-label="Close">\u2715</button>\n  <div class="lb-nav">\n    <button class="lb-btn lb-prev" aria-label="Previous">\u2039</button>\n    <button class="lb-btn lb-next" aria-label="Next">\u203A</button>\n  </div>\n  <div class="lb-caption"></div>\n</div>';
  ...
  prevBtn.addEventListener('click', ()=>{ if(index>0) openAt(index-1); });
  nextBtn.addEventListener('click', ()=>{ if(index<group.length-1) openAt(index+1); });
  ...
  document.addEventListener('click', (e)=>{
    ...
    group = $all('a[data-lightbox="gallery"]', container).filter(x=>/\.(png|jpg|jpeg|gif|webp)(\?|$)/i.test(x.getAttribute('href')||''));
